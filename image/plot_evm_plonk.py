"""
EVM: Direct + GKR + Plonk verifier baseline (gas). Data: image/evm.txt
Output: image/evm-plonk.png

Run: python image/plot_evm_plonk.py
"""
import sys
...
def main():
    ...
    ax.plot(
        N, direct_gas, linestyle="-", marker="o", color=COLOR_DIRECT,
        linewidth=LW, markersize=MS, label="Direct execution", zorder=3,
    )
    ax.plot(
        N, gkr_gas, linestyle="--", marker="s", color=COLOR_GKR,
        linewidth=LW, markersize=MS, label="GKR verification", zorder=3,
    )
    ax.plot(
        N, plonk_gas, linestyle="--", marker="^", color=COLOR_PLONK,
